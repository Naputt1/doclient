import type { Config, IR, IREndpoint, IRModule, IRError, IRParam, SourceAdapter } from '@doclient/core'
...
function normalizeType(shopeeType: string): string {
  switch (shopeeType) {
    case 'int': case 'int32': case 'int64': case 'timestamp': return 'integer'
    case 'float': case 'double': return 'number'
    case 'boolean': return 'boolean'
    case 'string': return 'string'
    case 'object': return 'object'
    case 'object[]': return 'object[]'
    case 'int[]': case 'int64[]': return 'integer[]'
    case 'string[]': return 'string[]'
    default: return shopeeType
  }
}

function mapParam(p: ShopeeParam, isRequest: boolean, typeOverrides?: Record<string, string>): IRParam {
...
export const shopeeSource: SourceAdapter = {
  ...
  async execute(config: Config): Promise<IR> {
    ...
    for (const mod of modules) {
      ...
      for (const item of mod.items) {
        ...
        const irEp: IREndpoint = {
          ...
          fullApiName: name,
          requestParams: requestParams.filter((p) => !isCommonParam(p.name)).map((p) => mapParam(p, true, typeOverrides)),
          responseParams: responseParams.map((p) => mapParam(p, false, typeOverrides)),
          errors: (info.error_list ?? []).filter((e) => e.name?.trim()).map((e) => ({ code: e.name.trim(), description: e.description ?? '' })),
        }
    ...
    const constants: Array<{ typeName: string; baseType: string; values: Array<{ name: string; value: string }> }> = []
    if (enumsDef) {
